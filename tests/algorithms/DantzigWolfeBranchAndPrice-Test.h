...
TEST(dantzig_wolfe_bap, optimal) {
    ...
    for (unsigned int i = 0 ; i < 5 ; i += 1) x(i).type(Binary);
...
TEST(dantzig_wolfe_bap, optimal_continuous_branching) {
    Environment env;
    VariableVector x = VariableVector(env, "x");
    VariableVector y = VariableVector(env, "y");
    ConstraintVector linking_constraints = ConstraintVector(env, "linking_constraints");
    ...
    for (unsigned int i = 0 ; i < 5 ; i += 1) x(i).priority(1);
...
TEST(dantzig_wolfe_bap, random) {
    for (unsigned int test = 0 ; test < 5 ; test += 1) {
        ...
        for (unsigned int j = 0 ; j < n_var_x ; j += 1) {
            objective.expression() += (1 + (rand() % 2)) * x(j);
            x(j).type(Binary);
        }
        for (unsigned int j = 0 ; j < n_var_y ; j += 1) objective.expression() += (rand() % 10) * y(j);
        for (unsigned int i = 0 ; i < n_linking_ctr ; i += 1) {
            for (unsigned int j = 0 ; j < n_var_x ; j += 1) linking_constraints(i).expression() += (rand() % 100) * x(j);
        ...
        for (unsigned int i = 0 ; i < n_subprob_x_ctr ; i += 1) {
            for (unsigned int j = 0 ; j < n_var_x ; j += 1) subproblem_x(i).expression() += (rand() % 100) * x(j);
